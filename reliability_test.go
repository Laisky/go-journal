...
func regressionReadOnlyFile(t *testing.T) *os.File {
	...
	if err := os.WriteFile(name, nil, 0600); err != nil { t.Fatal(err) }
	fp, err := os.Open(name)
...
func TestRegressionIDWritePropagatesFlushError(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := enc.Write(42); err == nil { t.Error("an ID write to a read-only file was reported as successful") }
}
...
func TestRegressionFailedIDWriteIsNotCommitted(t *testing.T) {
	...
	if err := j.WriteId(42); err == nil { t.Error("failed acknowledgement write returned no error") }
	if ids.CheckAndRemove(42) { t.Error("failed acknowledgement was published in the committed-ID index") }
}
...
func TestRegressionJournalPropagatesDataFlushError(t *testing.T) {
	for _, closeEncoder := range []bool{false, true} {
		enc, err := NewDataEncoder(regressionReadOnlyFile(t), false)
		if err != nil { t.Fatal(err) }
		if err := enc.writer.WriteInt64(42); err != nil { t.Fatal(err) }
		j := &Journal{dataEnc: enc}
		if closeEncoder { err = j.flushAndClose() } else { err = j.Flush() }
		if err == nil { t.Errorf("data flush failure lost (close=%v)", closeEncoder) }
	}
}
...
func regressionWriteIDs(t *testing.T, name string, ids ...int64) {
	...
	if err != nil { t.Fatal(err) }
	for _, id := range ids { if err := enc.Write(id); err != nil { t.Fatal(err) } }
...
func TestRegressionLegacyMaximumAcrossFiles(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if got != 99 { t.Errorf("maximum=%d, want 99 (not the maximum from only the last file)", got) }
}

func TestRegressionInt64SetCountsUniqueIDs(t *testing.T) {
	s := NewInt64Set()
	for i := 0; i < 10; i++ { s.AddInt64(42) }
	if got := s.GetLen(); got != 1 { t.Errorf("set length=%d after duplicate inserts, want 1", got) }
	if !s.CheckAndRemove(42) { t.Error("inserted ID missing") }
	if s.CheckAndRemove(42) { t.Error("removed ID still present") }
	if got := s.GetLen(); got != 0 { t.Errorf("set length=%d after removal, want 0", got) }
}
...
func TestRegressionTTLExpiredCountConcurrent(t *testing.T) {
	for round := 0; round < 100; round++ {
		...
		close(start)
		wg.Wait()
		if got := s.GetLen(); got != 0 { t.Fatalf("expired-ID count=%d, want 0", got) }
	}
}
...
func TestRegressionTTLRetainsAcknowledgementForMultipleCopies(t *testing.T) {
	...
	s.AddInt64(42)
	for i := 0; i < 3; i++ {
		if !s.CheckAndRemove(42) { t.Fatal("membership lookup consumed an acknowledgement needed by another replay copy") }
	}
	if got := s.GetLen(); got != 1 { t.Errorf("length=%d, want 1", got) }
}
...
func TestRegressionTTLCloseAfterWorkerStopped(t *testing.T) {
	...
	select { case <-done: case <-time.After(250*time.Millisecond): t.Error("Close blocks without a worker or on repeated calls") }
}
...
func TestRegressionJournalCloseWithoutWorkers(t *testing.T) {
	...
	select { case <-done: case <-time.After(250*time.Millisecond): t.Error("journal Close blocks before Start or after workers stop") }
}
...
func TestRegressionConcurrentIDEncoder(t *testing.T) {
	...
	for id := int64(1); id <= n; id++ {
		...
		go func(id int64) {
			defer wg.Done()
			<-start
			if err := enc.Write(id); err != nil { t.Errorf("write ID: %v", err) }
		}(id)
	}
	close(start)
	wg.Wait()
	...
	if err != nil { t.Fatal(err) }
	ids := NewInt64Set()
	if err := dec.ReadAllToInt64Set(ids); err != nil { t.Fatal(err) }
	for id := int64(1); id <= n; id++ { if !ids.CheckAndRemove(id) { t.Errorf("ID %d lost/corrupted", id) } }
}
