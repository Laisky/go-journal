	...
	for _, zipped := range []bool{false, true} {
		t.Run(fmt.Sprint(zipped), func(t *testing.T) {
			...
			for _, expected := range want {
				...
				if expected.ID != got.ID || got.Data["label"] != expected.Data["label"] || !bytes.Equal(expected.Data["body"].([]byte), got.Data["body"].([]byte)) {
					t.Fatalf("record %d corrupted across buffer boundary", expected.ID)
				}
			}
...
func TestPerformanceContractIDsAcrossBuffers(t *testing.T) {
	for _, zipped := range []bool{false, true} {
		t.Run(fmt.Sprint(zipped), func(t *testing.T) {
			...
			if err = dec.ReadAllToInt64Set(&got); err != nil {
				t.Fatal(err)
			}
			if !reflect.DeepEqual([]int64(got), ids) {
				t.Fatal("ID ordering/value changed")
			}
			if _, err = fp.Seek(0, 0); err != nil {
				t.Fatal(err)
			}
			...
			if err != nil || high != 9223372036854775807 {
...
func TestPerformanceContractTruncatedID(t *testing.T) {
	for tail := 1; tail < 8; tail++ {
		t.Run(fmt.Sprint(tail), func(t *testing.T) {
			...
			if _, err = fp.Write(raw[:8+tail]); err != nil {
				t.Fatal(err)
			}
			if _, err = fp.Seek(0, 0); err != nil {
				t.Fatal(err)
			...
			var got collectedIDs
			if err = dec.ReadAllToInt64Set(&got); err == nil {
				t.Fatal("partial ID accepted as EOF")
			}
			if !reflect.DeepEqual([]int64(got), []int64{91}) {
				t.Fatalf("valid prefix changed: %v", got)
			}
		})
	}
}
